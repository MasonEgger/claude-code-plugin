...
- **Exactly ONE commit per dispatch.** No follow-ups, no fixups, no amends, no `--no-verify`. Run the full test suite one final time before staging; visually confirm `git diff --staged` matches your intent; then commit ONCE. After `git commit` succeeds, you cannot fix anything in this dispatch — if you discover an issue post-commit, return a `Failure:` report describing what needs to happen. Do NOT make a follow-up commit; the orchestrator will not pick it up, and the user will handle resolution. (Some global pre-commit hooks reject any commit without a new `.ai-sessions/session-*.md` in non-interactive mode, which is fundamentally incompatible with multi-commit-per-step flows — the one-commit-per-step rule is the right answer, not `--no-verify`.)
...
- **Pre-flight: branch + clean-tree check.** BEFORE any work, run `git rev-parse --abbrev-ref HEAD` AND `git status --short`. Abort with `Failure:` if EITHER (a) the branch is `main` or `master`, OR (b) `git status --short` returns any output (dirty tree). A dirty tree at dispatch start means incomplete cleanup from a prior dispatch — or, more likely, that the orchestrator violated its SEQUENTIAL DISPATCHES ONLY contract and another subagent is running concurrently on the same repo. In either case: do not continue. Echo both command outputs in your Failure report so the orchestrator (and the user) can see what state you found.
...
2. **Branch + tree-state re-check.** Repeat the pre-flight checks from the invariant here as defense in depth: `git rev-parse --abbrev-ref HEAD` (abort on `main`/`master`) AND `git status --short` (abort if non-empty — concurrent subagent suspected, sequential-dispatch contract violated). Echo both command outputs in user-facing text.
...
5. **Commit message.** Read `${CLAUDE_PLUGIN_ROOT}/commands/commit-message.md` and execute its procedure inline. The output is `commit-msg.md` at the repo root. ALWAYS overwrite any existing `commit-msg.md` — it is gitignored and persists between commits, so a stale message from a prior commit will silently ride along into yours if you skip this step. Real bug source.
6. **Commit.** Before staging, run the project's test command one final time and confirm it exits 0 — this is your last chance to catch issues. Stage exactly these files — enumerate them, do not use `git add -A`:
   ...
   NEVER stage `commit-msg.md` (it is gitignored, but be explicit). After staging, run `git diff --staged --stat` and verify the file set matches (a)–(d) above. Then `git commit -S -F commit-msg.md`. Echo the resulting short SHA in user-facing text. If a pre-commit hook rejects the commit, capture its output verbatim in your `Failure:` report and stop — do NOT retry, do NOT amend, do NOT bypass with `--no-verify`, do NOT make a follow-up "fix" commit.
